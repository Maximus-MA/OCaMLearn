...
let load_cnn_mnist_full () =
  ...
  let reshape_images images =
    Array.map (fun image -> Array.init 1 (fun _ -> image)) images
  in
  let cnn_train_images = reshape_images train_images in
  ...
  let train_data = Ndarray.create 
      ...
      [| Array.length train_images; 1; 28; 28 |] in

  let train_labels = Ndarray.create 
      (Array.concat (Array.to_list train_labels)) 
      [| Array.length train_labels; 10 |] in
...
let load_cnn_mnist full_size = 
  if full_size then load_cnn_mnist_full ()
  else load_cnn_mnist_small ()